if plot_op && ~mod(step,plot_op)
    
    % get frame number and start preparing output
    frame = floor(step/plot_op);
    fprintf('\n*****  plot output snapshot %d for %s \n',frame,runID);
    ...
    if ~bnchmrk
        ...
        scatter(Pe(:),tau(:),20,log10(eta_vep(:)),'filled'); colorbar; colormap(ocean);
        ...
        fh7 = figure(7); clf;
        TT = linspace(0,1,1e3);
        [~,CCs,CCf] = equilibrium(TT,0.*TT,0.*TT,0.*TT,perT,perCs,perCf,clap,dTH2O,PhDg);
        ...
        plot(MAJ,T-Pt*clap+dTH2O*TRIf.^0.75,'k.',MAJs,T-Pt*clap+dTH2O*TRIf.^0.75,'b.',MAJf,T-Pt*clap+dTH2O*TRIf.^0.75,'r.','LineWidth',2,'MarkerSize',20);
    ...
    print(fh1,name,'-dpng','-r200');
    name = ['../out/',runID,'/',runID,'_mat_',num2str(frame)];
    print(fh2,name,'-dpng','-r200');
    name = ['../out/',runID,'/',runID,'_stc_',num2str(frame)];
    print(fh3,name,'-dpng','-r200');
    name = ['../out/',runID,'/',runID,'_sis_',num2str(frame)];
    print(fh4,name,'-dpng','-r200');
    if ~bnchmrk
        name = ['../out/',runID,'/',runID,'_flr_',num2str(frame)];
        print(fh6,name,'-dpng','-r200');
        name = ['../out/',runID,'/',runID,'_phs_',num2str(frame)];
        print(fh7,name,'-dpng','-r200');
    end

end

% save output frame
if save_op && ~mod(step,save_op)
    
    frame = floor(step/save_op);
    fprintf('\n*****  save output frame %d for %s \n',frame,runID);
