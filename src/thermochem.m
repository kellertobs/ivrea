...
if step > 0
    ...
    Div_fV(ic,ic) = flxdiv(1-f,U+UBG,W+WBG,h,'FLXDIV','flx');              % phase advection/compaction
    ...
    Rf    =  Div_fV + Lpl_f/PeC + RctR_f;                                  % total rate of change
    ...
    % update temperature
    V_GrdT(ic,ic) = flxdiv(T,U+u+UBG,W+w+WBG,h,'FROMM','adv');             % advection
    
    Lpl_T(ic,ic)  = (diff(T(:,ic),2,1)./h^2 ...
                  +  diff(T(ic,:),2,2)./h^2);                              % diffusion
    
    RT    = -V_GrdT + Lpl_T/PeT - RctR_f/St;                               % total rate of change
    ...
    Div_fMAJV(ic,ic) = flxdiv((1-f).*MAJs,U +UBG,W +WBG,h,'FLXDIV','flx') ...
    ...
    RMAJ    = -Div_fMAJV + Lpl_MAJ/PeC;                                    % total rate of change
    ...
    Div_fTRIV(ic,ic) = flxdiv((1-f).*TRIs,U +UBG,W +WBG,h,'FROMM','flx') ...
    ...
    RTRI    = -Div_fTRIV + Lpl_TRI/PeC;                                    % total rate of change
    ...
    Div_fTRCV(ic,ic) = flxdiv((1-f).*TRCs,U +UBG,W +WBG,h,'FROMM','flx') ...
    ...
    RTRC    = -Div_fTRCV + Lpl_TRC/PeC;                                    % total rate of change
    ...
    % update incompatible trace element composition
    Div_fIRPV(ic,ic) = flxdiv((1-f).*IRPs,U +UBG,W +WBG,h,'FROMM','flx') ...
                     + flxdiv(   f .*IRPf,uf+UBG,wf+WBG,h,'FROMM','flx');  % advection/compaction
    
    Lpl_IRP(ic,ic)  = (diff(IRP(:,ic),2,1)./h^2 ...
                    +  diff(IRP(ic,:),2,2)./h^2);                          % diffusion
    
    RIRP    = -Div_fIRPV + Lpl_IRP/PeC - DcyR_IR;                          % total rate of change
    ...
    % update compatible trace element composition
    Div_fIRDV(ic,ic) = flxdiv((1-f).*IRDs,U +UBG,W +WBG,h,'FROMM','flx') ...
                     + flxdiv(   f .*IRDf,uf+UBG,wf+WBG,h,'FROMM','flx');  % advection/compaction
    
    Lpl_IRD(ic,ic)  = (diff(IRD(:,ic),2,1)./h^2 ...
                    +  diff(IRD(ic,:),2,2)./h^2);                          % diffusion
    
    RIRD    = -Div_fIRDV + Lpl_IRD/PeC + DcyR_IR;                          % total rate of change
    ...
    V_GrdISS(ic,ic) = flxdiv(ISS,U+UBG,W+WBG,h,'FROMM','adv');             % advection/compaction
    
    Lpl_ISS(ic,ic)  = (diff(ISS(:,ic),2,1)./h^2 ...
                    +  diff(ISS(ic,:),2,2)./h^2);                          % diffusion
    
    RISS    = -V_GrdISS + Lpl_ISS/PeC - (ISR-ISS).*RctR_f./max(flim,1-f);  % total rate of change
    ...
    V_GrdISF(ic,ic) = flxdiv(ISF,uf+UBG,wf+WBG,h,'FROMM','adv');             % advection/compaction
    
    Lpl_ISF(ic,ic)  = (diff(ISF(:,ic),2,1)./h^2 ...
                    +  diff(ISF(ic,:),2,2)./h^2);                          % diffusion
    
    RISF    = -V_GrdISF + Lpl_ISF/PeC + (ISR-ISF).*RctR_f./max(flim,f);    % total rate of change
